...
lemma range_cfc_nnreal (a : A) (ha : 0 ≤ a) :
  ...
  obtain (⟨h₁, h₂⟩ | h | h) : (ContinuousOn f (σ ℝ a) ∧ IsSelfAdjoint a) ∨ ¬ ContinuousOn f (σ ℝ a) ∨ ¬ IsSelfAdjoint a := by tauto
  · refine ⟨f, ?_, rfl⟩
...
lemma range_cfcₙ_nnreal (a : A) (ha : 0 ≤ a) :
  ...
  simp only [Set.preimage_setOf_eq, Set.mem_setOf_eq, Set.mem_image] at hf ⊢
  obtain (⟨h₁, h₂, h₃⟩ | h | h | h) :
      (ContinuousOn f (σₙ ℝ a) ∧ f 0 = 0 ∧ IsSelfAdjoint a) ∨ ¬ ContinuousOn f (σₙ ℝ a) ∨
        f 0 ≠ 0 ∨ ¬ IsSelfAdjoint a := by tauto
  · refine ⟨f, ?_, rfl⟩
