...
/- First we have to prove that if `0 ≤ a ≤ e` then `a = eae`. To do this on paper, we consider
the CStarAlgebra in the unitization of A generated by `1, e, eae`. This clearly contains `a`,
(or the image of a in the unitization). In the Gelfand rep of this commutative CStarAlgebra we
have that `0 ≤ a ≤ e` translates into the support of the function `a` being a subset of the domain
of the characteristic function `e`. From this we see `eae = a` in the unitization, a fact which
descends to the original CStarAlgebra. We should be able to handle this
with Jireh's trick. -/

/- How do we consider the abelian `CStarAlgebra` generated by these three things? Maybe we don't
need to. Here is the meat of Jireh's comment:
...
/- However, in order to do any of this, we do need the `A` to be a `CommCStarAlgebra`, so it
appears we need to at least embed the `CStarAlgebra` generated by `1,e,eae` into such a thing.

We first can realize the `StarSubalgebra` generated by this set using
...
and then show it is commutative using
...
/- For practice, let's first do this without any reference to the unitization. -/

open StarAlgebra

variable {A : Type*} [CStarAlgebra A] [PartialOrder A] [StarOrderedRing A]
variable (a e : A) (h0a : 0 ≤ a) (he : IsStarProjection e) (hae : a ≤ e)

abbrev subalg := adjoin ℂ { x : A | x = 1 ∨ x = e ∨ x = e * a * e }

theorem hComm (a e : A) (h0a : 0 ≤ a) (he : IsStarProjection e) (hae : a ≤ e)
    : ∀ x y, x ∈ { x : A | x = 1 ∨ x = e ∨ x = e * a * e } →
    y ∈ { x : A | x = 1 ∨ x = e ∨ x = e * a * e } → x * y = y * x := by
  intro x y hx hy
  aesop?
