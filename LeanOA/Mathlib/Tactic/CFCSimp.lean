...
structure Config where
  ...
  /-- Use the real lemmas, deferring their hypotheses as side goals, instead of the `sorry`d
  hypothesis-free copies. -/
  real : Bool := false
...
algebra. -/
def instantiateTarget (R : Expr) (t : Target) (e : Entry) (name : Name) (inv : Bool)
    (S : Expr := R) : MetaM (Option (Expr × Bool)) := do
  ...
  let body ← if inv then mkEqSymm body else pure body
...
def userTheorem (real : Bool) (declName : Name) : MetaM (Expr × Bool) := do
  ...
  forallTelescopeReducing info.type fun xs body => do
    ...
    for x in xs do
      ...
      if !real && !decl.binderInfo.isInstImplicit && (← isProp decl.type) &&
...
partial def getSetup (cfg : Config) (t : Expr) (extra : Array Name) (disch : Simp.Discharge)
  ...
  for n in extra do
    let (prf, inv) ← userTheorem cfg.real n
  ...
  for e in entries do
    let (name, inv) := if cfg.real then (e.origName, e.inv) else (e.declName, false)
    match e.kind with
    | .pull =>
    ...
    | .target =>
      for (tg, i) in targets.zipIdx do
        ...
        for S in #[R] ++ dist.map (mkConst ·.1) do
          let some (prf, bare) ← instantiateTarget R tg e name inv S | continue
  ...
  let procs := getSetup cfg t extra disch cache stack
...
/-- In `+real` mode: discharge a hypothesis with a local hypothesis if there is one, and otherwise
with a placeholder. `simp` rejects a rewrite whose proof contains an assignable metavariable, so the
placeholder is a `sorry`, marked so that `replacePlaceholders` can turn it into a goal afterwards. -/
...
def elabArgs (cfgStx : TSyntax ``optConfig) (lems? : Option (TSyntax ``cfcSimpLemmas))
  ...
  let disch := if cfg.real then deferDischarge !cfg.defer else Simp.dischargeDefault?
  let s ← getSetup cfg t extra disch (← IO.mkRef #[]) (← IO.mkRef #[]) R
  return (cfg, s)
