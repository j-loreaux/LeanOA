...
example {A B : Type*} [CStarAlgebra A] [CStarAlgebra B]
    {a : A} (ha : IsSelfAdjoint a) (φ : A →⋆ₐ[ℂ] B) (hφ : Function.Injective φ) :
    spectrum ℝ (φ a) = spectrum ℝ a := by
  -- one subset inclusion is immediate since `φ` is an algebra homomorphism.
  refine Set.Subset.antisymm (AlgHom.spectrum_apply_subset (φ.restrictScalars ℝ) a) fun x hx ↦ ?_
  /- we prove the reverse inclusion by contradiction, so assume that `x ∈ spectrum ℝ a`, but
  `x ∉ spectrum ℝ (φ a)`. Then by Urysohn's lemma we can get a continuous function for which
  `f x = 1`, but `f = 0` on `spectrum ℝ a`. -/
  by_contra hx'
...
example {A B : Type*} [CStarAlgebra A] [CStarAlgebra B]
  ...
  -- Since `a` and `φ a` are selfadjoint, their `ℂ`-spectra are the images of their `ℝ`-spectra,
  -- which coincide by the previous result.
...
example [NonUnitalCStarAlgebra A] [NonUnitalCStarAlgebra B]
  ...
  For selfadjoint elements, the spectral radius over `ℝ` coincides with the norm,
  hence `‖ψ (star a * a)‖ = ‖star a * a‖`. -/
