...
/-!  # The `cfc_pull` test suite, run against `cfc_simp +real` (unmodified Mathlib lemmas) -/
...
example (ha : IsSelfAdjoint a) : NormedSpace.exp a = cfc Real.exp a := by cfc_simp +real ℝ a

example (ha : 0 ≤ a) : 1 - CFC.sqrt a = cfc (fun x ↦ 1 - √x) a := by cfc_simp +real ℝ a
...
example (f : ℝ≥0 → ℝ≥0) (hf0 : f 0 = 0)
  ...
  cfc_simp +real -unital ℝ≥0 (CFC.sqrt (a ^ 2))

example (ha : 0 ≤ a) : CFC.sqrt a * CFC.sqrt a = cfc (fun x : ℂ ↦ x.sqrt * x.sqrt) a := by
  cfc_simp +real [CFC.sqrt_eq_cfc_complex_sqrt] ℂ a

example : a⁺ - a⁻ = cfcₙ (fun x : ℝ ↦ x⁺ - x⁻) a := by
  cfc_simp +real -unital ℝ a
...
example (ha : IsSelfAdjoint a) : (2 • a)⁺ = cfcₙ (fun x : ℝ ↦ (2 • x)⁺) a := by cfc_simp +real ℝ a
...
example (ha : IsStrictlyPositive a) :
    CFC.log a * CFC.log a = cfc (fun x : ℝ ↦ Real.log x * Real.log x) a := by
  cfc_simp +real ℝ a => all_goals first | (exact Real.continuousOn_log.mono fun x hx h ↦ spectrum.zero_notMem ℝ ha.2 (h ▸ hx))

example (ha : IsSelfAdjoint a) (f : ℝ → ℝ) (hf : Continuous f)
    (hspec : spectrum ℝ a ⊆ Set.Icc (-1) 1) (hf0 : ∀ x ∈ Set.Icc (-1 : ℝ) 1, f x ≠ 0) :
    Ring.inverse (cfc f a) = cfc (fun x : ℝ ↦ (f x)⁻¹) a := by
  cfc_simp +real +zetaDelta ℝ a => all_goals first | (fun_prop) | (intro x hx; exact hf0 x (hspec hx)) | exact ha

example (f : ℝ → ℝ) (hf : Continuous f) (hf0 : 0 = f 0) :
    cfcₙ f a + cfcₙ f a = cfcₙ (fun x ↦ f x + f x) a := by
  cfc_simp +real -unital ℝ a => all_goals first | (symm; exact hf0)
...
example (ha : IsStarNormal a) : star a * a = cfc (fun x : ℂ ↦ star x * x) a := by
  cfc_simp +real [cfc_star_id, cfc_id', cfc_mul] ℂ a
...
example (x y : ℝ) (hx : 0 ≤ x) (hy : 0 ≤ y) :
    a ^ x * a ^ y = cfc (fun t : ℝ≥0 ↦ t ^ x * t ^ y) a := by
  cfc_simp +real +defer [CFC.rpow_def, cfc_mul] ℝ≥0 a => all_goals fun_prop
...
example (ha : IsStarNormal a) (b : A) : star a * a + b = cfc (fun x : ℂ ↦ star x * x) a + b := by
  conv in star a * a =>
    cfc_simp +real +defer [cfc_star_id, cfc_id', cfc_mul] ℂ a =>
      -- `IsStarNormal a` is a class, so `simp` finds `ha` by instance synthesis: no predicate goal
      all_goals fun_prop
...
example [Nontrivial A] (ha : IsSelfAdjoint a) (ha_norm : ‖a‖ ≤ 1) :
  ...
  cfc_simp +real ℂ a =>
    ...
    have key : (1 : A) - a ^ 2 = cfc (fun x : ℝ ↦ 1 - x ^ 2) a := by cfc_simp +real ℝ a
    rw [key]
...
example (ha : 0 ≤ a) :
  ...
  cfc_simp +real ℝ≥0 a => all_goals first | (simp)
...
example (f g : ℝ → ℝ) (hf : ContinuousOn f (spectrum ℝ a))
    ...
    cfc f a - cfc g a = cfc (fun x ↦ f x - g x) a := by
  cfc_simp +real +defer ℝ a => all_goals assumption
...
example (ha : IsStarNormal a) :
  ...
  cfc_simp +real ℂ (a : Unitization ℂ A)
