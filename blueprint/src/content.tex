...
\begin{proof}
  By Lemma \ref{lem: pos_iff_star_mul_self (Sak 1.4.4)}, $a^{*}ha=a^{*}x^{*}xa=(xa)^{*}(xa)\ge 0$.
\end{proof}
...
\begin{lemma}
  ...
  \uses{lem: sigma_cont_of_normal (Sak 1.7.4),lem: proj_of_sup_proj,cor: proj_sub_of_subproj}
...
\begin{proof}
  ...
  $p-q_0$ is a nonzero subprojection of $p$ by Lemma \ref{cor: proj_sub_of_subproj}, and there exists a nonzero projection $q_{1}\le p-q_{0}$
...
\begin{lemma}
  \label{lem: zorn_base}
  \uses{cor: proj_sub_of_subproj,lem: selfadjoint_le_norm,lem: sigma_cont_of_normal (Sak 1.7.4),lem: proj_of_sup_proj}
...
\begin{proof}
  ...
  By Lemma \ref{cor: proj_sub_of_subproj} we know $p-p_\alpha$ is a projection. By the $C^{*}$--property
