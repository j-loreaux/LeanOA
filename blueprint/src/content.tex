...
\begin{lemma}
  ...
  \lean{IsStarProjection.sub_iff_mul_eq_left,IsStarProjection.sub_iff_mul_eq_right}
...
\begin{lemma}
    ...
    $0 \le f \le 1$ is a directed set with respect to the order on $C(K)$.
\end{lemma}
...
\begin{lemma}
    \label{lem:sigma_closed_star_subalg_wstar}
...
\begin{lemma}
  \label{lem:cut_down_sigma_closed_cts}
  Let $e$ be any projection in $M$. Then the subalgebra $eMe$ is $\sigma$--closed and the mapping $x\mapsto exe$ is $\sigma$--continuous.
\end{lemma}
