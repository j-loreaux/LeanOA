...
\begin{lemma}
  ...
  \lean{CStarAlgebra.nonneg_TFAE,StarOrderedRing.nonneg_TFAE}
...
\begin{lemma}
  \label{lem:uw_pos_sep_pts} (Sak 1.7.2)
...
of a function in $C(K)$ is just it's range. So $b\le a$ iff $b(t)\le a(t)$ for all $t\in K$. A net
 $(f_{\alpha})$ in $C(K)$ is \textit{bounded} if there is a positive constant that bounds the norms of all $f_{\alpha}$.
A net is said to be \textit{increasing} if $\alpha \le \beta$ implies $f_{\alpha}\le f_{\beta}$ for all $\alpha,\beta$
...
\begin{proof}
    The usual order is a partial order, and given $f,g\in C(K)$ we have $f\vee g \in C(K)$, where $f\vee g(t)=\max{f(t),g(t)}$. 
    This function is also supported on $U$.
\end{proof}
