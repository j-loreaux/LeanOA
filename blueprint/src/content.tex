...
\begin{lemma}
  \label{lem: bdd_sigma_cts_iff_bdd_s_cts}
...
\begin{proof}
  ...
  $A$ is star isomorphic to $C(K)$, where $K$ is Stonean by Lemmas \ref{lem: cut_down_of_wstar} and \ref{lem: spec_masa_wstar_stonean}. 
  By Lemma \ref{lem: fin_lin_approx_of_stonean} it follows that $\varphi(a)\le \psi(a)$ for every $a\ge 0$ in $A$, which holds a fortiori for $a\ge 0$ in $C^{*}(x^{*}x,p)$. 
  ...
  \end{align}
  Since $x\mapsto \varphi(xp_0)$ is $\sigma$--continuous, it is $s$-continuous by Lemma \ref{lem: bdd_sigma_cts_iff_bdd_s_cts}. The seminorm
