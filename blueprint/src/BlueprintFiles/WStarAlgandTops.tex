\chapter{\boldmath\texorpdfstring{$C^{*}$}{CStar}-- Algebras and \boldmath\texorpdfstring{$W^{*}$}{WStar}-- Algebras}
...
\begin{lemma}
    \label{lem:sigma_closed_star_subalg_wstar_Sak_1_1_4} (Sakai 1.1.4)
...
%  \mathlibok
%  \uses{def:cstar_def}
%  Let $\mathcal{A}$ be a $C^{*}$--algebra, and $a\in \mathcal{A}$ such that $a$ is self-adjoint
%  ($a = a^{*}$) or normal ($a^{*}a=aa^{*}$). Let $\mathcal{A}_{1}$ denote the unitization of
%  $\mathcal{A}$. A complex number $\lambda$ is in $q\sigma(a)$, the \textit{quasispectrum} of $a$,
%  if $\lambda - a$ is not invertible in $\mathcal{A}_{1}$. There is an isometric $*$--homomorphism of
%  $C(q\sigma(a))$ into $\mathcal{A}$ called the \textbf{continuous functional calculus}.
%\end{theorem}
...
\begin{proposition}
  ...
  \lean{mem_extremePoints_isSelfAdjoint_and_mem_closedUnitBall_iff_isSelfAdjoint_and_mem_unitary}
  \leanok
...
\begin{lemma}
  ...
  \leanok
  \lean{IsCompact.extremePoints_nonempty,CStarAlgebra.ofExtremePt}
...
\begin{lemma}
  \label{lem:cut_down_sigma_closed_cts_Sak_1_7_6} (Sakai 1.7.6)
  \uses{def:sigma_top}
...
\begin{lemma}
  ...
  \uses{def:sigma_top}
  \uses{lem:cut_down_sigma_closed_cts,lem:sigma_closed_star_subalg_wstar_Sak_1_1_4}
...
\begin{proof}
  ...
  $e$ is a self-adjoint projection implies that $eMe$ is a $*$--algebra. By Lemma \ref{lem:sigma_closed_star_subalg_wstar_Sak_1_1_4}, if this
...
\begin{lemma}
  ...
  \uses{def:sigma_top,lem:cut_down_sigma_closed_cts_Sak_1_7_6}
...
\begin{lemma}
  ...
  \uses{def:sigma_top,lem:fin_lin_approx_of_stonean_Sak_1_3_1,lem:spec_masa_wstar_stonean_Sak_1_7_5,lem:left_mul_proj_right_mul_proj_sig_cts_Sak_1_7_7}
...
\begin{lemma}
  ...
  \uses{cor:sig_clos_of_cstar_is_wstar_Sak_1_7_9,lem:sigma_closed_star_subalg_wstar_Sak_1_1_4}
